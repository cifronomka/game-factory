...
3. Игрок повышает `heat`, открывает семь стадий мира, получает всё более высокий множитель и учится менять ритм: ровные нажатия заряжают вспышку, короткий интенсивный burst использует её, передышка восстанавливает эффективность.
4. Начиная с третьей стадии мир отвечает телеграфируемыми помехами. Они не добавляют новое управление: игрок либо быстро нажимает, либо выдерживает короткую паузу согласно ясному визуальному сигналу.
5. После первого входа в Инферно цель меняется с раскрытия сцены на удержание жара и улучшение `Best Score` и времени удержания Инферно.
6. Когда пламя полностью угасает, игрок получает экран результата с очками, достигнутой стадией, временем Инферно, личными рекордами и одной основной кнопкой «Разжечь снова».
7. Rewarded-предложение «Печать Инферно ×2» доступно как добровольный power mode, но отказ, отсутствие SDK или ошибка рекламы не блокируют рестарт и обычную прогрессию.
...
- Управление остаётся одношаговым, но оптимальный ритм меняется: размеренные нажатия, короткая вспышка и передышка чередуются с читаемыми атаками персонажей.
...
- Rewarded-реклама оформлена как зрелищное добровольное усиление, а не как спасение из искусственно созданного поражения.
...
- Программирование, production-ассеты, анимации, SDK-интеграция и release не входят в текущий planning stage.
- Базовая механика должна быть полностью доступна без рекламы, авторизации, leaderboard и сети. Отсутствующая внешняя capability деградирует до локального результата, а не до ошибки.
...
- Tap-target центрального огня и основные кнопки должны иметь интерактивную область не менее 44×44 CSS px.
...
|---|---|---|---|---|---|
| Однообразное бесконечное нажатие | Высокая | Высокое | более 50% тестеров называют второй минутный отрезок повторением первого; одинаковая частота ввода на протяжении >30 с | цикл «резонанс → вспышка → передышка», телеграфируемые события на стадиях 3/5/6+, смена цели после входа в Инферно | Product/Game Designer |
| Физическая усталость от чрезмерного tapping | Высокая | Высокое | медиана >6 эффективных taps/с в течение 30 с либо жалоба на усталость у >20% тестеров | soft-cap на 5 полноценных taps/с, убывающая отдача спама, бонусы за ритм и паузу; баланс прохождения на 3–5 taps/с | Product/Game Designer + QA |
| Все семь стадий раскрываются слишком быстро | Средняя | Высокое | >35% новых игроков достигают Инферно быстрее 90 с | целевое первое достижение 150–300 с; корректировать пороги, decay и события, не скрытые задержки | Product/Game Designer |
| Инферно кажется недостижимым | Средняя | Высокое | <8% первых завершённых сессий достигают стадии 7 | onboarding до стадии 2, читаемые телеграфы, снижение decay/дебаффов по данным funnel при сохранении ритма | Product/Game Designer + QA |
...
| Rewarded boost воспринимается как обязательный или pay-to-win | Средняя | Высокое | >40% eligible-сессий используют boost и score без boost отстаёт >35% при сопоставимом skill proxy | лимит 1/run, отсутствие прямых score points за assisted heat и revive/блокировки, достижимость Инферно без рекламы, отдельная аналитика boosted runs | Monetization + Product |
...
| Time to first Inferno | медиана активного времени до первого входа в стадию 7 среди достигших её | N/A | 150–300 с | первые 300 достижений | game events |
...
| Input comfort | сессии без непрерывного 30-секундного отрезка со средней частотой >6 accepted taps/с / валидные сессии | N/A | ≥90% | первые 1 000 сессий | input telemetry, агрегировано |
| Rewarded opt-in | подтверждённые показы / сессии, где placement был доступен | N/A | 10–35% | rolling 14 days, min 500 eligible | ad + game events |
...
Без персональных данных должны различаться: `interactive_ready`, `session_start`, `stage_enter`, `stage_leave`, `resonance_ready`, `surge_start`, `surge_end`, `enemy_event_start`, `enemy_event_result`, `inferno_enter`, `inferno_exit`, `session_end`, `personal_best`, `rewarded_offer`, `rewarded_start`, `rewarded_complete`, `rewarded_cancel`, `rewarded_error`. Для boosted run хранится булев признак `boost_used`; частота tap передаётся агрегатами, а не сырым таймлайном.

## Решения и открытые вопросы

| Статус | Вопрос/решение | Владелец | Срок |
|---|---|---|---|
| Решено | Название 1.0 — «Зажги», slug — `inferno-clicker`; семь исходных названий и порядок стадий сохранены. | Product/Game Designer | planning stage |
| Решено | Главный публичный leaderboard содержит только `Best Score`; highest stage, longest Inferno hold и max multiplier — локальные показатели профиля/экрана результата. | Product/Game Designer + Platform | planning stage |
| Решено | Базовая игра и достижение Инферно не требуют рекламы; «Печать Инферно ×2» — добровольный boost после stage 3 и 45 секунд active run, максимум один успешный boost за run. Assisted heat не даёт прямых tap-score points. | Product/Game Designer | planning stage |
| Решено | Первый release — один бесконечный score-run до полного угасания; после Инферно игрок продолжает удержание ради рекорда. | Product/Game Designer | planning stage |
| Решено | Retention строится на личных рекордах и ежедневном ритуале без серии посещений, валюты и штрафа за пропуск. | Product/Game Designer | planning stage |
| Открыто | Подтвердить фактический рейтинг, рекламные и leaderboard-требования по актуальным правилам Яндекс Игр. | Platform Integration Agent | до implementation platform adapter |
| Открыто | Уточнить KPI и баланс по результатам минимум 10 внутренних playtest и 30 внешних first-session playtest. | Product/Game Designer + QA | до release candidate |

## Handoff

- Измеримые правила и исходный баланс находятся в `GAME_DESIGN.md`; QA должен трассировать stage thresholds, scoring, rhythm, enemy events, pause и persistence.
...
- Art/Audio должны сделать каждую стадию и каждый ритмический/вражеский сигнал различимыми даже при muted audio либо reduced motion.
