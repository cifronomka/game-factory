...
- Этап: Corrective Cycle 06; direct-tap V5/concurrent-hazard gameplay, platform contract, flame/environment design и audio заморожены. Активная работа ограничена character animation/edges/effects, silent adaptive quality и bounded clip residency. Cycle 05 остаётся историей и не закрывает Cycle 06 gate.
...
- Ash Servant keeps one stationary hearth-side root. The 1.0 s telegraph is split into prepare/inhale/hold; the 2.5 s effect into exhale start/ramp/peak/fade/end followed by a separately authored 800 ms presentation-only recovery using all eight cells. Appearance, idle, inhale, blow and recovery contain unique ordered in-betweens; reverse playback and full-pose ghost crossfade are forbidden. Per-frame root/mouth sockets drive a bounded scarlet-snowflake emitter from mouth to `FlameRig.getTargetAnchor()`.
- Demoness source resolution supports the largest DPR-2 render at effective upscale `≤1.25×`. Appearance/idle/disapproval/cast/hold/recovery use unique forward-authored cells and per-frame hand sockets. A bounded parametric shard stream advances blue conical shards toward the live flame mask; `impactStrength` stays zero until the leading shard is within the 12 px contact tolerance. The leading shard disappears at that threshold and steam starts from its sampled point; steam rises/fades and all presentation clocks freeze with pause.
...
- Demoness v5 uses `400×600` source cells and a `460×700` logical placement, keeping the largest required DPR-2 render at or below `1.25×` effective upscale. Silhouette/reveal, calm idle, presentation-only disapproval, cast, hold and 1.0 s recovery each traverse all eight authored cells; alpha/root geometry and hand sockets are versioned in atlas metadata.
