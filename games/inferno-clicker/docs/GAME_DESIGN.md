...
## Cycle 06 presentation-only clarification

Числовые правила, расписания и причинность событий ниже не меняются. Cycle 06 заменяет только их визуальное выражение:

- `Порыв слуги`: те же `1,0 с` telegraph и `2,5 с` effect; стабильный Ash Servant вдыхает и выдыхает из покадрового mouth socket поток алых снежинок в текущую видимую область пламени. Снежинки не являются новым input, hit или gameplay modifier.
- `Холодное клеймо`: те же `2,0 с` telegraph и `4,0 с` effect; Demoness выпускает из текущих hand sockets синие конусообразные льдины. Визуальная реакция огня начинается только после пересечения льдины с текущей flame mask; в точке контакта льдина испаряется и создаёт ограниченный пар. Gameplay effect всё так же начинается по core schedule и не зависит от Canvas collision.
- На pause все presentation clocks, projectiles и steam замораживаются без catch-up; cancel/teardown очищает их. Reduced Motion и low quality сохраняют всю смысловую последовательность, уменьшая только необязательную плотность.
...
- Визуально слуга остаётся цельным и стабильным, затем выдыхает изо рта алые снежинки в живую точку пламени; телеграф и gameplay timing от количества кадров не зависят.
...
- Визуально каст читается как `руки → синие конусообразные льдины → контакт с пламенем → испарение/поднимающийся пар`; fire reaction до контакта запрещена.
