...
Документ задаёт порядок реализации `inferno-clicker`. Active Corrective Cycle 06 сохраняет завершённые M1–M4/M7–M10 и замороженные V5 gameplay/audio/platform contracts. Он переоткрывает presentation milestones M5/M6, asset residency M13 и три QA/release gates M11–M15. `CORRECTIVE_CYCLE_06.md` имеет precedence над конфликтующими Cycle 05 presentation решениями.
...
| M5-C06 | Character asset production | Preserve retained Servant/Demoness identity/key poses; produce clean-alpha clip atlases with genuine in-betweens and higher-resolution Demoness; author internal host motion while preserving entry; metadata roots/mouth/hands/hashes/provenance | M4 + approved Cycle 06 art/asset contracts | Unique-hash/landmark/edge/upscale audits PASS; reverse/duplicate/permutation not counted; no white matte |
| M6-C06 | Character runtime and FX | Remove distant full-pose ghost blend; drive Servant scarlet snowflakes from mouth; Demoness conical ice projectiles/contact evaporation/steam; authored host animation; silent refresh-aware adaptive quality; preserve low/reduced semantics | M5-C06 | Normal/slow temporal tests, socket/target/contact lifecycle, toast=0, quality matrix, bounded pools/pause/cleanup PASS |
...
| M13-C06 | Bounded clip residency/performance | Partition large animations into disposable clips; ahead-of-need preload, active/imminent pin, deterministic release; measure update/render separately from refresh; optimize without rule/audio changes | M6-C06–M12 exact candidate | Instant residency target≤56 MiB/hard≤64 MiB, spikes>50 ms after preload=0, no active release/missing flash/leak; 10-minute trace PASS |
...
Corrective order: freeze gameplay/audio fingerprints → produce/version C06 character clips and metadata → integrate stable authored animation → implement snowflake and ice/contact/steam semantics → preserve entry while replacing static host motion → implement silent render-cost quality controller → partition/preload/release clip resources → automated/visual Pass 1 → independent blind Pass 2 → fix/retest → full Pass 3 regression → release gate. Cycle 02–05 screenshots do not close Cycle 06.
